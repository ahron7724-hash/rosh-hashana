...
הדף נעול מאחורי קוד משפחתי. מי שנכנס בלי הקוד רואה מסך חסימה בלבד ("אל תתחפרן").
מי שמקליד את הקוד — הוא נשמר ב-localStorage של אותו מכשיר לתמיד (עד "נעילת הדף
במכשיר הזה" בתפריט ⋯). קישור מהיר למשפחה: `…/?code=<הקוד>` — נכנס אוטומטית ומנקה
את הכתובת.

בקוד המקור נשמר רק ה-SHA-256 של הקוד (מנורמל: אותיות וספרות בלבד, קטנות), לא הקוד
עצמו. לשינוי הקוד: לעדכן את `GATE_HASH` ב-[src/App.jsx](src/App.jsx) ל-hash החדש
(`echo -n "הקודהחדש" | sha256sum`, אחרי הסרת רווחים/פיסוק). זו חסימת-תצוגה — לא
הגנה ברמת מסד הנתונים.
